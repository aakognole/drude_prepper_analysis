...
def read_box(psf, filename):
    s = numpy.loadtxt(filename, dtype='str') #json.load(open(filename, 'r'))
    boxlx, boxly, boxlz = float(s[0,2]), float(s[1,2]), float(s[2,2])  #map(float, sysinfo['dimensions'][:3])
    print("boxlx, boxly, boxlz",boxlx, boxly, boxlz)
